...
/**
 * GET /admin/users
 * List all registered users with role details
 */
...
/**
 * POST /admin/users
 * Create a new user with dynamic role assignment
 */
async function createUser(req, res) {
  const { username, password, name, role, role_id } = req.body;
  ...
  const insertRes = await query(
    `INSERT INTO users (username, password_hash, name, role, role_id, is_active)
     ...
     RETURNING id, username, name, role, role_id, is_active, created_at`,
    [cleanUsername, passwordHash, name.trim(), targetRole.slug, targetRole.id]
  );
...
/**
 * PUT /admin/users/:id
 * Update user details & dynamic role
 */
async function updateUser(req, res) {
  const { id } = req.params;
  const { name, role, role_id, is_active } = req.body;

  const userCheck = await query('SELECT id, username, role, role_id FROM users WHERE id = $1', [id]);
  ...
  const updateRes = await query(
    `UPDATE users
     SET name = COALESCE($1, name),
         ...
         updated_at = NOW()
     WHERE id = $5
     RETURNING id, username, name, role, role_id, is_active, updated_at`,
    [newName, newRoleSlug, newRoleId, newIsActive, id]
  );
...
async function deleteUser(req, res) {
  ...
  logger.info(`Pengguna '${userRes.rows[0].username}' telah dinonaktifkan oleh ${req.user.username}`);
